...
timestamp_gl = ['timestamp_',datestr(datetime,'yyyymmdd_HHMM')];
filepath_stored_gl = ['output_files_patients/',timestamp_gl];
...
for i = 1:nmc
    clear mc_objective
    disp(['Starting iteration ',num2str(i),' out of ',num2str(nmc)]);
...
ratioe = xmc(ismember(rxn_full,'PG3e == SERe'),:)./xmc(ismember(rxn_full,'SERx == SERe'),:);
ration = xmc(ismember(rxn_full,'PG3n == SERn'),:)./xmc(ismember(rxn_full,'SERx == SERn'),:);
ratiob = xmc(ismember(rxn_full,'PG3b == SERb'),:)./xmc(ismember(rxn_full,'SERx == SERb'),:);

lower_bounds = [prctile(ratioe,alpha);prctile(ration,alpha);prctile(ratiob,alpha)];
upper_bounds = [prctile(ratioe,100-alpha);prctile(ration,100-alpha);prctile(ratiob,100-alpha)];

fluxes = xopt_min(1:nflux);
vale = fluxes(ismember(rxn_full,'PG3e == SERe'),:)./fluxes(ismember(rxn_full,'SERx == SERe'),:);
valn = fluxes(ismember(rxn_full,'PG3n == SERn'),:)./fluxes(ismember(rxn_full,'SERx == SERn'),:);
valb = fluxes(ismember(rxn_full,'PG3b == SERb'),:)./fluxes(ismember(rxn_full,'SERx == SERb'),:);


table_new = table(["Enhancing";"Non-Enhancing";"Brain"],[vale;valn;valb],lower_bounds,upper_bounds);

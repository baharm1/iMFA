...
lb(n_flux_param + 1) = conc_values(3, 1); % lb for amp conc
lb(n_flux_param + 2) = conc_values(4, 1); % lb for gdp conc
%lb(n_flux_param + 3) = conc_values(5, 1); % gmp; uncomment for brain
lb(n_flux_param + 4) = conc_values(2, 1); % lb for guanosine conc
lb(n_flux_param + 6) = conc_values(1, 1); % lb for inosine conc

ub = 1000 * ones(n_flux_param+n_c_param,1);
ub(1) = 100; % de novo imp

% upper bounds for concentrations
ub(n_flux_param + 1) = 1.5*conc_values(3, 1); % lb for amp conc
ub(n_flux_param + 2) = 1.5*conc_values(4, 1); % lb for gdp conc
%ub(n_flux_param + 3) = 1.5*conc_values(5, 1); % gmp; uncomment for brain
ub(n_flux_param + 4) = 1.5*conc_values(2, 1); % lb for guanosine conc
ub(n_flux_param + 6) = 1.5*conc_values(1, 1); % lb for inosine conc

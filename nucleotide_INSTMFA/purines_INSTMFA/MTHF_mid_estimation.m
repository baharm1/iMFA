%% Read the input data
input_data_file = '..\..\Purine-DMFA\Input-Data\Input_gbm_rt_norm.xlsx';
min_sd = sqrt(10^(-5));
...
for t = 2:length(time_points)
    ...
    [mthf_mid(:,t), x_opt] = calc_methf_mid(ser_MID, gly_MID, ser_sd, gly_sd,[]);
    ...
    for n = 1:nmc
      [mthf_mc(:,n),~] = calc_methf_mid(sim_mid(1:4,n), sim_mid(5:end,n), ser_sd, gly_sd,x_opt);
    end
    
    % Calaculate the mean and SD of the simulated distrubution
    mthf_mid(:,t) = mean(mthf_mc,2);
    mthf_sd(:,t) = std(mthf_mc,0,2);
...
% Add the data for time = 0 (unlabeled case) 
 ...
 mthf_mid = 100*mthf_mid;
 mthf_sd = 100*mthf_sd;
 results = table(["M0";"M1"],mthf_mid,mthf_sd);
 writetable(results, [input_data_file,'_mthf_calc.xlsx']);


function [mthf_mid, x_opt] = calc_methf_mid(ser_MID, gly_MID, ser_sd, gly_sd,x0_opt)
...
mthf_mid0 = x_opt(1) + x_opt(2) + x_opt(5);
mthf_mid1 = x_opt(3) + x_opt(4) +x_opt(6);
mthf_mid = [mthf_mid0; mthf_mid1];

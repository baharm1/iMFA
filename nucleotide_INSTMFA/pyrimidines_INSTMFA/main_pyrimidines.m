...
model_file = 'pyrimidine_model';

% File containing experimental data
input_data_file = 'Input-Data\Input_gbm_ctrl_norm.xlsx'; % GBM data
% if you run it for cortex, please follow the instructions in dMdt_MID.m to
% comment/uncomment lines related to cortex:
% input_data_file = 'Input-Data\Input_brain_ctrl_norm.xlsx'; % cortex data
...
ub = 200 * ones(n_flux_param + n_c_param, 1);
ub(1) = 50; % de novo UMP
ub(2) = 200; % uridine == ump
ub(3) = 100; % ump == uridine 
ub(4) = 400; % cytidine == uridine
ub(5) = 200; % UMP_out
ub(6) = 400; % uridine out

ub(n_flux_param + 1:n_flux_param + n_c_param) = 1.5 * conc_values(:, 1);
...
nparam = numel(lb);
ncons = size(Aeq, 1) + numel(conc_metab) + n_input_param + ...
    sum(~isnan(MID_balance(:, 2:end)),'all'); 
dof = ncons - nparam;
...
x0(2, :) = 100 * rand(1, niter); % uridine_ump
x0(3, :) = 10 * rand(1, niter); % ump_uridine
x0(4, :) = 100 * rand(1, niter); % cytidine_uridine
x0(5, :) = 100 * rand(1, niter); % UMP out
x0(6, :) = 100 * rand(1, niter); % uridine out

% Use the input MID mean values as initial guess
x0 = [x0; repmat(input_mid_array, 1, niter)]; 

% concentrations
x0(n_flux_param + 1:n_flux_param + n_c_param, :) = (ub(n_flux_param + ...
    1:n_flux_param + n_c_param) - lb(n_flux_param + 1:n_flux_param + ...
    n_c_param)) .* rand(n_c_param, niter) + lb(n_flux_param + 1:n_flux_param + ...
    n_c_param);

% aspartate fraction
x0(n_flux_param + n_c_param + 1:n_flux_param + n_c_param + ...
    n_isotopomer_frac,:) = (ub(n_flux_param + n_c_param + 1:n_flux_param + ...
    n_c_param + n_isotopomer_frac) - lb(n_flux_param + n_c_param + 1:n_flux_param + ...
    n_c_param + n_isotopomer_frac)) .* rand(n_isotopomer_frac, niter) + lb(n_flux_param + ...
    n_c_param + 1:n_flux_param + n_c_param + n_isotopomer_frac);
    
[x, fval, exitflag] = estimate_model_param(Aeq, beq, tspan, time_points, ...
    n_flux_param, n_c_param, n_isotopomer_frac, MID_balance, ...
    balance_data_metabs, SD_balance, balance_metabs, ...
    MID_input, input_data_metabs, SD_input, conc_metab, conc_values, ...
    lb, ub, niter, x0, 'parallel', ncores);
...
save([folder,'\data']);
value = [x_opt(1:n_flux_param + n_c_param + n_isotopomer_frac); 
    fval_range(minimum)];

parameter = [rxns; "c_uridine"; "c_ump"; 
    "f_aspartate_M1"; "f_aspartate_M2"; "f_aspartate_M3"; "min_objective"];
...
writetable(table(parameter,[x_range(1:n_flux_param + n_c_param + ...
    n_isotopomer_frac, :); fval_range]), ...
